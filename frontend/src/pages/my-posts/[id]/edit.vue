...
async function loadInitialData() {
  ...
  try {
    ...
    if (postId.value) {
      const post: Post = await postsApi.getPost(postId.value);
      ...
      // Восстанавливаем тип видимости для «Общих»
      ...
      // Восстанавливаем единый статус из status_global / status_guild
...
async function submit() {
  ...
  try {
    ...
    await postsApi.updatePost(postId.value, payload);
...
watch(
  ...
  (newVal) => {
    ...
    // Если гильдия уже выбрана (например, при редактировании), не переназначаем её
    if (guildId.value) {
      return;
    }
    // Привязка только к гильдии выбранного персонажа
    const ch = selectedCharacter.value;
    if (ch?.guild) {
...
watch(
  ...
  async (newId) => {
    if (!newId) return;
    if (guildPermissionSlugs.value[newId]) {
      if (!canPublishToGuild.value) {
        isVisibleGuild.value = false;
      }
      return;
    }
  ...
  <div class="container py-8 md:py-12">
    <div class="mx-auto max-w-3xl space-y-6">
      <div class="flex items-center justify-between gap-3">
        <div>
          <h1 class="mb-1 text-3xl font-bold tracking-tight">Редактирование поста</h1>
          <p class="text-muted-foreground">
            Измените содержимое и видимость поста.
          </p>
        </div>
      ...
      <div class="flex items-center justify-between gap-4">
        ...
        <div class="ml-auto flex gap-3">
          ...
          <Button :disabled="loading" @click="submit">
            {{ loading ? 'Сохранение…' : 'Сохранить пост' }}
          </Button>
        </div>
      </div>
    </div>
