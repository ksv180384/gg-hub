<script setup>
import { ref } from 'vue';
import { Users, UserCog, Key, Swords } from 'lucide-vue-next';

const items = ref([
  {
    key: 'games', // Уникальный ключ для категории
    label: 'Игры',
    // icon: 'pi pi-users',
    // icon_el: Swords,
    // badge: 5,
  ...
  {
    key: 'users', // Уникальный ключ для категории
    label: 'Пользователи',
    // icon: 'pi pi-users',
    // icon_el: Users,
    // badge: 5,
...
const expandedKeys = ref({ users: true });
</script>
              ...
              >
                <template #item="{ item }">
                  <a v-ripple class="flex items-center px-4 py-2 cursor-pointer group">
                    <template v-if="item.icon_el">
                      <component :is="item.icon_el" :size="16"></component>
                    </template>
                    ...
                    <span
                      v-if="item.items"
                      class="pi pi-angle-right text-primary ml-auto transition-all duration-300 ease-in-out"
