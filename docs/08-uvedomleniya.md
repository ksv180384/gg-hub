...
In-app оповещения хранятся три месяца с момента создания. По истечении этого срока они автоматически удаляются независимо от статуса прочтения.

Очистка выполняется ежедневно командой `php artisan notifications:prune-expired`, зарегистрированной в `routes/console.php`.
...
In-app уведомления создаются в двух точках:

- после подачи заявки — лидеру выбранного рейда;
- после принятия или отклонения — владельцу персонажа-заявителя.

Оба уведомления ведут на `/guilds/{guildId}/raids`. Подробный поток описан в [raids.md](raids.md).
